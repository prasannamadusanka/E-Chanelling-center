...
//    private static final  String updateClinicdosge = "UPDATE `suwasewana_db`.`vaccine_clinic_session` SET `max_patient` = ? WHERE (`vcs_id` = ?);\n";
    ...
    private static final String USER_HOME_VIEW_ANNOUNCEMENTS = "SELECT * FROM `normal_clinic_session` AS cs LEFT JOIN `clinic_registered_patient` AS cp ON cs.ncs_id=cp.ncs_id WHERE u_nic IS NULL OR `u_nic` !=? AND `target_moh`=? LIMIT 2;";
    //    private static final  String getUserTpMohIt="SELECT uMobile,uMoh FROM suwasewana_db.user where uNic=?;";
    ...
    private static final String USER_CHECK_REGISTER_DISEASE = "SELECT* FROM `user_register_disease` WHERE d_id=? AND UNic=?";
...
//            System.out.println(rs);
    ...
    public String UserRegistration(UserRegistrationModel userRegister) {
        System.out.println("data come to dao 1");
...
//            return throwables.getMessage();
    ...
    public String userregisterdisease(String name, String UNic, String d_id) {
...
//        rs = preparedStatement.executeQuery();
    ...
    public ArrayList<UserRegisterDiseaeseModel> usercheckregisterdisease(UserRegisterDiseaeseModel userdisease,String uNic, String d_id) throws SQLException {

        try (PreparedStatement preparedStatement = connection.prepareStatement(USER_CHECK_REGISTER_DISEASE)) {
            preparedStatement.setString(2, d_id);
            preparedStatement.setString(1, uNic);
            ResultSet rs = preparedStatement.executeQuery();
            ArrayList<UserRegisterDiseaeseModel> viewregdiseasedetails = new ArrayList<UserRegisterDiseaeseModel>();
            System.out.println("data dao");
            while (rs.next()) {
                String id = rs.getString("d_id");
                String name = rs.getString("name");
                String date = rs.getString("date");
                UserRegisterDiseaeseModel temp = new UserRegisterDiseaeseModel(
                        id,
                        name,
                        date
                );

                viewregdiseasedetails.add(temp);
            }
            return viewregdiseasedetails;
